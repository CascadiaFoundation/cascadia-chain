...
To [submit the proposal](../../users/governance/submitting.md) to testnet through the command line with [`cascadiad`](../../validators/quickstart/binary.md), use the following command with `register-erc20`:

```bash
cascadiad tx gov submit-proposal register-erc20 <erc20-address> \
  --title=<title> \
...
```bash
cascadiad tx gov submit-proposal register-erc20 --proposal=<path/to/proposal.json>
```
