...
Find below a list of  `cascadiad` commands added with the  `x/erc20` module. You can obtain the full list by using the `cascadiad -h` command. A CLI command can look like this:

```bash
cascadiad query erc20 params
```
...
```bash
cascadiad tx gov submit-proposal register-coin [metadata] [flags]
```
...
```bash
cascadiad tx gov submit-proposal register-erc20 [erc20-address] [flags]
```
...
```bash
cascadiad tx gov submit-proposal toggle-token-conversion [token] [flags]
```
